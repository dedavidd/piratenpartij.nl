			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					else
					{
						...
						$oPaynl->setReturnUrl(idealcheckout_getRootUrl() . 'return.php?idealcheckout_order_id=' . urlencode($sOrderId) . '&idealcheckout_order_code=' . urlencode($sOrderCode));
						$oPaynl->setReportUrl(idealcheckout_getRootUrl() . 'report.php?idealcheckout_order_id=' . urlencode($sOrderId) . '&idealcheckout_order_code=' . urlencode($sOrderCode));
		...
		public function doValidate()
		{
			...
			if(idealcheckout_database_num_rows($oRecordset))
			{
				while($oRecord = idealcheckout_database_fetch_assoc($oRecordset))
				{
					...
					$this->oRecord['transaction_status'] = $oPaynl->getStatus($oRecord['transaction_id']);
