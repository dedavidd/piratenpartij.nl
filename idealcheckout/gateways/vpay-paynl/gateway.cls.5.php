			...
			else
			{
				...
				if($this->getRecordByOrder())
				{
					...
					else
					{
						...
						$oPaynl->setReturnUrl(idealcheckout_getRootUrl() . 'return.php?order_id=' . urlencode($sOrderId) . '&order_code=' . urlencode($sOrderCode));
						$oPaynl->setReportUrl(idealcheckout_getRootUrl() . 'report.php?order_id=' . urlencode($sOrderId) . '&order_code=' . urlencode($sOrderCode));
		...
		public function doValidate()
		{
			...
			if(idealcheckout_database_num_rows($oRecordset))
			{
				while($oRecord = idealcheckout_database_fetch_assoc($oRecordset))
				{
					...
					$this->oRecord['transaction_status'] = $oPaynl->doStatusRequest($oRecord['transaction_id']);
