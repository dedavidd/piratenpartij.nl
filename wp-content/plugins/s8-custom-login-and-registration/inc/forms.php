            ...
            case 'forgot':
                ...
                ?><form name="forgot" action="<?php echo s8_get_forgot_password_url(); ?>" method="post" class="s8_form forgot_form"><?php
                    ...
                    <p>
                        ...
                        if ( ! $args['use_html5_placeholders'] ) { // Don't display if we are using html5 placeholders
                            ?><label for="user_login"><?php _e( 'Gebruikersnaam of e-mailadres' ); ?></label><?php
                        }
                        if ( $args['split_lines'] && ! $args['single_line'] ) echo '<br/>'; ?>
                        <input type="text" name="user_login" id="user_login" value="<?php echo $_POST['user_login']; ?>" <?php if ( $args['use_html5_placeholders'] ) echo 'placeholder="' . __( 'Gebruikersnaam of e-mailadres' ) . '"'; ?> /><?php
                        if ( ! $args['single_line'] ) echo '<br/>';

                        // SUBMIT BUTTON AND LINKS ?>
                        <input type="submit" name="s8-login-forgot" value="Herstellen" />
                        <?php
                        if ( $args['login'] ) {
                ...
                ?><form action="<?php echo s8_get_login_url( $redirect_url ); ?>" name="login" method="post" class="s8_form login_form"><?php
                    ...
                    <p>
                        ...
                        if ( ! $args['hide_remember'] ) { ?>
                            <input type="checkbox" id="remember-me" name="remember" value="forever" /> <label for="remember-me"><? _e( 'Onthoud mij' ); ?></label><?php
                            if ( ! $args['single_line'] ) echo '<br/>';
                        }

                        // SUBMIT BUTTON AND LINKS ?>
                        <input type="hidden" name="redirect" value="<?php echo $redirect_url; ?>" />
                        <input type="submit" name="s8-login" value="<?php echo ( $args['submit_text'] ) ? $args['submit_text'] : __( 'Aanmelden' ); ?>" class="login-button" />
                        <?php
                        if ( $args['forgot'] ) {
