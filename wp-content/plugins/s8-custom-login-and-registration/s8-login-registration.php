...
class s8_login_registration {
    ...
    private $version = '0.8.7',
            $title = 'Niet gevonden',
            $content = 'Sorry, deze pagina kon niet gevonden worden!';
        ...
        elseif(isset($wp_query->query_vars[self::ep_login])) {
            ...
            switch($_GET['action']) {
                ...
                case 'reset':
                    ...
                    $this->title = __( 'Wachtwoord vergeten' );
    ...
    function register_user() {
        ...
        if(empty($_POST['username']) || empty($_POST['email']))
            $errors = new WP_Error('emptyfield', 'Een vereist veld is leeggelaten. Alle velden zijn vereist.');
        elseif(!is_email(trim($_POST['email'])))
            $errors = new WP_Error('invalidemail', 'Het opgegeven e-mailadres is ongeldig.');
        elseif(email_exists(trim($_POST['email'])))
            $errors = new WP_Error('emailexists', 'Er bestaat al een account met dit e-mailadres. Alle accounts moeten een uniek e-mailadres hebben.');
        elseif(!validate_username(trim($_POST['username'])))
            $errors = new WP_Error('invalidusername', 'De gekozen gebruikersnaam is ongeldig. Zorg ervoor dat je gebruikersnaam begint met een letter, geen spaties of bijzondere tekens bevat en minstens 5 tekens lang is.');
        elseif(username_exists(trim($_POST['username'])))
            $errors = new WP_Error('userexists', 'De gekozen gebruikersnaam bestaat al. Probeer een ander.');
        else {
            ...
            else $errors = new WP_Error('nouser', 'Er was een fout tijdens het registreren.');
        }
        return $errors;
    }
    ...
    function user_password_reset() {
        ...
        elseif(empty($_POST['new-pass']) || !preg_match('/^[a-zA-Z0-9\.!#@$%^&?*]{8,30}$/', $_POST['new-pass'])) $errors = new WP_Error('invalid', 'Wachtwoord is ongeldig. Wachtwoorden moeten minstens 8 tekens lang zijn en geen spaties, (), {}, &gt;&lt; of [] bevatten.');
        // Does the confirm password match the new password?
        elseif($_POST['new-pass'] != $_POST['new-pass-confirm']) $errors = new WP_Error('nomatch', 'Wachtwoorden komen niet overeen. Probeer het nog eens.');
