...
{
//	return get_permalink(94) ? get_permalink(94) : "https://lidworden.piratenpartij.nl";
	return "https://lidworden.piratenpartij.nl";
}
...
function the_bootstrap_register_scripts_styles() {
	...
	if ( ! is_admin() ) {
		...
		wp_register_script(
			'tw-bootstrap',
			get_template_directory_uri() . "/static/vendor/bootstrap-2.3.1/js/bootstrap{$suffix}.js",
			array('jquery'),
			'2.0.3',
			true
		);
...
function the_bootstrap_print_ie_scripts() {
	?>
	<!--[if lt IE 9]>
		<script src="https://piratenpartij.nl/javascript/twitter-bootstrap/html5shiv/html5shiv.min.js" type="text/javascript"></script>
		<script src="<?php echo get_template_directory_uri(); ?>/static/vendor/respond/respond.min.js" type="text/javascript"></script>
	<![endif]-->
	<?php
}
	...
	if ( have_comments() ) : ?>
		<div id="comments">
			<h2 id="comments-title">
				<?php printf( _n( 'E&eacute;n reactie op &ldquo;%2$s&rdquo;', '%1$s reacties op &ldquo;%2$s&rdquo;', get_comments_number(), 'the-bootstrap' ),
...
function the_bootstrap_comments_closed() {
	if ( ! is_page() AND post_type_supports( get_post_type(), 'comments' ) ) : ?>
		<p class="nocomments"><?php _e( 'Reacties zijn gesloten.', 'the-bootstrap' ); ?></p>
	<?php endif;
}
