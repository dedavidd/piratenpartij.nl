...
function the_bootstrap_comment_nav() {
	...
	<nav class="comment-nav well">
		<h1 class="assistive-text"><?php _e( 'Navigeren door reacties', 'the-bootstrap' ); ?></h1>
		<div class="nav-previous alignleft"><?php next_comments_link( __( '&larr; Nieuwere reacties', 'the-bootstrap' ) ); ?></div>
		<div class="nav-next alignright"><?php previous_comments_link( __( 'Oudere reacties &rarr;', 'the-bootstrap' ) ); ?></div>
	</nav>
...
function the_bootstrap_posted_on() {
	printf( __( '<span class="sep">Gepubliceerd op </span><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s" pubdate>%4$s</time></a> door <a href="<?php get_author_posts_url; ?>"><?php get_the author; ?></a>', 'the-bootstrap' ),
			esc_url( get_permalink() ),
			esc_attr( get_the_time() ),
			esc_attr( get_the_date( 'c' ) ),
			esc_html( get_the_date() ),
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
			esc_attr( sprintf( __( 'Toon alle artikelen door %s', 'the-bootstrap' ), get_the_author() ) ),
			get_the_author()
	);
	if ( comments_open() AND ! post_password_required() ) { ?>
		<span class="sep"> | </span>
		<span class="comments-link">
			<?php comments_popup_link( '<span class="leave-reply">' . __( 'Reageren', 'the-bootstrap' ) . '</span>', __( '<strong>1</strong> reactie', 'the-bootstrap' ), __( '<strong>%</strong> reacties', 'the-bootstrap' ) ); ?>
		</span>
		<?php
	}
	edit_post_link( __( 'Bewerken', 'the-bootstrap' ), '<span class="sep">&nbsp;</span><span class="edit-link label">', '</span>' );
}
endif;
...
function the_bootstrap_link_pages( $args = array() ) {
	...
	$defaults = array(
		...
		'nextpagelink'		=> __('Volgende pagina', 'the-bootstrap'),
		'previouspagelink'	=> __('Vorige pagina', 'the-bootstrap'),
		'pagelink'			=> '%',
		'echo'				=> true
	);
	...
	if ( $multipage ) {
		if ( 'number' == $next_or_number ) {
			$output .= '<nav class="pagination clear"><ul><li><span class="dots">' . __('Pagina#39;s:', 'the-bootstrap') . '</span></li>';
...
function the_bootstrap_navbar_searchform( $echo = true ) {
	$searchform = '	<form id="searchform" class="navbar-search pull-right" method="get" action="' . esc_url( home_url( '/' ) ) . '">
						<label for="s" class="assistive-text hidden">' . __( 'Zoeken', 'the-bootstrap' ) . '</label>
						<input type="search" class="search-query" name="s" id="s" placeholder="' . esc_attr__( 'Zoeken', 'the-bootstrap' ) . '" />
					</form>';
...
function the_bootstrap_comments_link( $zero = false, $one = false, $more = false, $css_class = '', $none = false ) {
	...
	if ( false === $zero ) $zero = __( 'Geen reacties' );
	if ( false === $one  ) $one  = __( '1 reactie' );
	if ( false === $more ) $more = __( '% reacties' );
	if ( false === $none ) $none = __( 'Reacties uitgeschakeld' );
	...
	if ( post_password_required() ) {
		echo '<span' . $class . '>' . __( 'Voer je wachtwoord in om reacties te tonen.' ) . '</span>';
		return;
	}
	...
	$link = sprintf( '<a href="%1$s"%2$s%3s title="%4$s">%5$s</a>',
		...
		esc_attr( sprintf( __( 'Reageren op %s' ), the_title_attribute( array('echo' => 0 ) ) ) ),
