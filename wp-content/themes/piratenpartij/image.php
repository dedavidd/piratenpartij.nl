	...
	<div id="content" role="main">
		...
		<nav id="nav-single" class="well clearfix">
			<span class="gallery-link pull-left">
				...
				<?php printf(
					_x( '&laquo; %1$s (%2$s)', 'Titel, aantal afbeeldingen', 'the-bootstrap' ),
					get_the_title( $post->post_parent ),
					sprintf(
						_nx( '%d afbeelding', '%d afbeeldingen', $total_images, 'Aantal afbeeldingen', 'the-bootstrap' ),
						$total_images
					)
				); ?>
				</a>
			...
			<span class="nav-links pull-right">
				<?php
				edit_post_link( __( 'Bewerken', 'the-bootstrap' ), ' <span class="edit-link label">', '</span><span class="sep">&nbsp;</span>' );
				the_bootstrap_comments_link( __( 'Reageren', 'the-bootstrap' ) );
				if ( isset($attachments[$k-1]) )
					echo ' &mdash; <a href="' . get_permalink( $attachments[$k-1]->ID ) . '">' . __( '&laquo; Vorige afbeelding', 'the-bootstrap' ) . '</a>';
				if ( isset($attachments[$k+1]) )
					echo ' &mdash; <a href="' . get_permalink( $attachments[$k+1]->ID ) . '">' . __( 'Volgende afbeelding &raquo;', 'the-bootstrap' ) . '</a>';
				?>
			</span>
		</nav><!-- #nav-single -->
